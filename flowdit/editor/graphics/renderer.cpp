...
namespace flowdit::editor {
    Renderer::Renderer(WindowPlatform& platform, const int cuda_device) : window{platform}, instance{std::array{vk::KHRSurfaceExtensionName, vk::KHRWin32SurfaceExtensionName}}, surface{instance.instance, vk::Win32SurfaceCreateInfoKHR{{}, GetModuleHandleW(nullptr), window.native_window}}, device{instance, cuda_device}, resources{device} {
        ...
        for (int i = 0; i < 2; ++i) {
            std::ifstream file{std::filesystem::path{FLOWDIT_SHADER_DIRECTORY} / (std::string{names[i]} + ".spv"), std::ios::binary | std::ios::ate};
        ...
        device.logical.writeSamplerDescriptorsEXT(vk::SamplerCreateInfo{{}, vk::Filter::eNearest, vk::Filter::eNearest, vk::SamplerMipmapMode::eNearest, vk::SamplerAddressMode::eClampToEdge, vk::SamplerAddressMode::eClampToEdge, vk::SamplerAddressMode::eClampToEdge}, vk::HostAddressRangeEXT{static_cast<std::byte*>(resources.sampler_heap.mapped) + device.heap_properties.samplerDescriptorSize, device.heap_properties.samplerDescriptorSize});
        ImGui::CreateContext();
        auto& io               = ImGui::GetIO();
    ...
    std::uint64_t Renderer::texture(const vk::Extent2D extent, const vk::Format format) {
        ...
        auto& value            = textures.emplace(id, Texture{graphics::Image{device, extent, format}}).first->second;
    ...
    void Renderer::draw() {
        ...
        if (data.TotalVtxCount) {
            ...
            if (frame.vertices.size < vertex_bytes) frame.vertices = graphics::Buffer{device, std::bit_ceil(vertex_bytes), true};
            if (frame.indices.size < index_bytes) frame.indices = graphics::Buffer{device, std::bit_ceil(index_bytes), true};
        ...
        for (const auto* list : data.CmdLists) {
            for (const auto& draw : list->CmdBuffer) {
                ...
                } push{{std::uint32_t(frame_index * 2), 0}, {std::uint32_t(frame_index * 2 + 1), 0}, {std::uint32_t(draw.GetTexID() - 1), 0}, {std::uint32_t(draw.GetTexID() >> 32), 0}, index_offset + draw.IdxOffset, vertex_offset + draw.VtxOffset, {2 / data.DisplaySize.x, 2 / data.DisplaySize.y}, {-1 - data.DisplayPos.x * 2 / data.DisplaySize.x, -1 - data.DisplayPos.y * 2 / data.DisplaySize.y}};
