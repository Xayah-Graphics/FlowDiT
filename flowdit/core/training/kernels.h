...
namespace flowdit::kernels {
    ...
    void flow_matching_loss(::cuda::stream_ref stream, const float* prediction, const float* target, float* prediction_gradient, float* sample_loss, float* loss, std::uint32_t batch, std::uint32_t sample_elements, float gradient_scale);
