...
namespace flowdit::kernels {
    ...
    void make_training_batch(::cuda::stream_ref stream, const std::uint8_t* images, const std::uint32_t* dataset_labels, float* path, float* target, float* times, std::uint32_t* labels, const std::uint64_t* step, const std::uint64_t* seed, std::uint32_t batch, ImageLayout image, std::uint32_t image_count, std::uint32_t class_count, bool horizontal_flip);
    void make_sampling_noise(::cuda::stream_ref stream, float* state, std::uint64_t seed, std::uint32_t batch, std::uint32_t image_element_count);
    ...
    void flow_matching_loss(::cuda::stream_ref stream, const float* prediction, const float* target, float* prediction_gradient, float* sample_loss, float* loss, std::uint32_t batch, std::uint32_t image_element_count);
    ...
    void make_labels(::cuda::stream_ref stream, std::uint32_t* labels, std::uint32_t batch, std::uint32_t class_index, std::uint32_t class_count);
    ...
    void unpatchify(::cuda::stream_ref stream, const float* patches, std::uint8_t* rgba, std::uint32_t batch, ImageLayout image);
} // namespace flowdit::kernels
#endif
