...
namespace {
    ...
    flowdit::SamplingSolver parse_solver(const std::string_view name) {
        if (name == "euler") return flowdit::SamplingSolver::euler;
        if (name == "heun") return flowdit::SamplingSolver::heun;
        if (name == "rk4") return flowdit::SamplingSolver::rk4;
        throw std::runtime_error{"Unknown ODE solver."};
    }
    std::string_view solver_name(const flowdit::SamplingSolver solver) {
        switch (solver) {
        case flowdit::SamplingSolver::euler: return "euler";
        case flowdit::SamplingSolver::heun: return "heun";
        case flowdit::SamplingSolver::rk4: return "rk4";
        }
        std::unreachable();
    }
    ...
    void write_png(const std::filesystem::path& path, const flowdit::SamplingResult& result) {
    ...
    int train(const std::span<char*> arguments) {
        ...
        flowdit::Cifar10TrainingSet dataset{dataset_directory};
        flowdit::Trainer trainer{dataset, device_ordinal, seed};
        if (resume) {
        ...
        while (trainer.state.step < end_step) {
            ...
            const flowdit::TrainingStatistics statistics = trainer.optimize(iteration_count);
            ...
            std::fflush(stdout);
            if (step % 1'000u == 0u) {
                const flowdit::SamplingRequest request;
                const flowdit::SamplingResult parameter_result               = trainer.sample(request, flowdit::ParameterSource::parameters);
                const flowdit::SamplingResult exponential_average_result     = trainer.sample(request, flowdit::ParameterSource::exponential_average);
    ...
    int sample(const std::span<char*> arguments) {
        const std::chrono::steady_clock::time_point command_begin = std::chrono::steady_clock::now();
        flowdit::SamplingRequest request;
        ...
        std::fflush(stdout);
        flowdit::Sampler sampler{arguments[2], 0};
        ...
        std::fflush(stdout);
        const flowdit::SamplingResult result = sampler.sample(request);
